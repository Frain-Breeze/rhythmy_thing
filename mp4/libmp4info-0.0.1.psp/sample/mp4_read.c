...
char *mp4_read_get(struct mp4_read_struct *p, unsigned int packet, unsigned int audio_stream, struct mp4_read_output_struct *output) {
	...
	pspDebugScreenPrintf("%d, %d, %d, %d, %d, %d, %d, %d\n",
		packet,
		p->interleaving.output_audio_frame_number,
		p->interleaving.output_audio_frame_number + p->interleaving.output_number_of_audio_frames - 1,
		first_audio_frame,
		last_audio_frame,
		output->number_of_audio_frames,
		output->number_of_skip_audio_parts,
		output->number_of_audio_parts);
...
//		pspDebugScreenPrintf("%d, %d len0=%d, len1=%d\n", 
//			i,
//			first_audio_frame+i,
//			track->stsz_sample_size_table[first_audio_frame+i],
//			p->audio_output_length[i]);
		result = get_audio_sample(p, audio_stream, first_audio_frame+i, &audio_buffer);
		if (result != 0) {
			return(result);
		}
//		pspDebugScreenPrintf("%d, %d len0=%d, len1=%d ok\n", 
//			i,
//			first_audio_frame+i,
//			track->stsz_sample_size_table[first_audio_frame+i],
//			p->audio_output_length[i]);
